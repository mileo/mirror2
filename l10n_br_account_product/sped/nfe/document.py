...
class NFe200(FiscalDocument):
    ...
    def _serializer(self, cr, uid, ids, nfe_environment, context=None):
        ...
        for inv in pool.get('account.invoice').browse(cr, uid, ids, context):
            ...
            self._emmiter(
                cr, uid, ids, inv, company, context)
            self._receiver(
                cr, uid, ids, inv, company, nfe_environment, context)
            ...
            for inv_line in inv.invoice_line:
                ...
                for inv_di in inv_line.import_declaration_ids:
                    ...
                    for inv_di_line in inv_di.line_ids:
                        ...
                        self._adiction(cr, uid, ids, inv, inv_line, inv_di,
                                       inv_di_line, i, context)
    ...
    def _nfe_identification(self, cr, uid, ids, inv, company, nfe_environment,
                            context=None):
        ...
        self.nfe.infNFe.ide.indPag.valor = (inv.payment_term and
                                            inv.payment_term.indPag or '0')
        ...
        if inv.partner_shipping_id:
            if inv.partner_id.id != inv.partner_shipping_id.id:
                if self.nfe.infNFe.ide.tpNF.valor == '0':
                    self.nfe.infNFe.retirada.CNPJ.valor = re.sub(
                        '[%s]' % re.escape(string.punctuation), '',
                        inv.partner_shipping_id.cnpj_cpf or '')
                    ...
                    self.nfe.infNFe.retirada.xCpl.valor = \
                        inv.partner_shipping_id.street2 or ''
                    self.nfe.infNFe.retirada.xBairro.valor = \
                        inv.partner_shipping_id.district or 'Sem Bairro'
                    ...
                    self.nfe.infNFe.retirada.UF.valor = \
                        inv.partner_shipping_id.state_id.code or ''
                else:
                    self.nfe.infNFe.entrega.CNPJ.valor = re.sub(
                        '[%s]' % re.escape(string.punctuation), '',
                        inv.partner_shipping_id.cnpj_cpf or '')
    ...
    def _nfe_references(self, cr, uid, ids, inv_related, context=None):
        ...
        if inv_related.document_type == 'nf':
            self.nfref.refNF.cUF.valor = \
                inv_related.state_id and inv_related.state_id.ibge_code or '',
            self.nfref.refNF.AAMM.valor = datetime.strptime(
                inv_related.date, '%Y-%m-%d').strftime('%y%m') or ''
            self.nfref.refNF.CNPJ.valor = re.sub(
                '[%s]' % re.escape(string.punctuation), '',
                inv_related.cnpj_cpf or '')
            ...
            self.nfref.refNFP.cUF.valor = (inv_related.state_id and
                                           inv_related.state_id.ibge_code or
                                           '')
            self.nfref.refNFP.AAMM.valor = datetime.strptime(
                inv_related.date, '%Y-%m-%d').strftime('%y%m') or ''
            self.nfref.refNFP.IE.valor = re.sub(
                '[%s]' % re.escape(string.punctuation), '',
                inv_related.inscr_est or '')
            ...
            if inv_related.cpfcnpj_type == 'cnpj':
                self.nfref.refNFP.CNPJ.valor = re.sub(
                    '[%s]' % re.escape(string.punctuation), '',
                    inv_related.cnpj_cpf or '')
            else:
                self.nfref.refNFP.CPF.valor = re.sub(
                    '[%s]' % re.escape(string.punctuation), '',
                    inv_related.cnpj_cpf or '')
    ...
    def _emmiter(self, cr, uid, ids, inv, company, context=None):
        ...
        # Emitente
        #
        self.nfe.infNFe.emit.CNPJ.valor = re.sub(
            '[%s]' % re.escape(string.punctuation), '',
            inv.company_id.partner_id.cnpj_cpf or '')
        self.nfe.infNFe.emit.xNome.valor = (
            inv.company_id.partner_id.legal_name[:60])
        ...
        self.nfe.infNFe.emit.enderEmit.CEP.valor = re.sub(
            '[%s]' % re.escape(string.punctuation), '',
            str(company.zip or '').replace(' ', ''))
        ...
        self.nfe.infNFe.emit.enderEmit.fone.valor = re.sub(
            '[%s]' % re.escape(string.punctuation), '',
            str(company.phone or '').replace(' ', ''))
        self.nfe.infNFe.emit.IE.valor = re.sub(
            '[%s]' % re.escape(string.punctuation), '',
            inv.company_id.partner_id.inscr_est or '')
        self.nfe.infNFe.emit.IEST.valor = ''
        self.nfe.infNFe.emit.IM.valor = re.sub(
            '[%s]' % re.escape(string.punctuation), '',
        ...
        if inv.company_id.partner_id.inscr_mun:
            self.nfe.infNFe.emit.CNAE.valor = re.sub(
                '[%s]' % re.escape(string.punctuation), '',
                inv.company_id.cnae_main_id.code or '')

    def _receiver(self, cr, uid, ids, inv, company, nfe_environment,
                  context=None):
        ...
        if nfe_environment == '2':
            ...
            self.nfe.infNFe.dest.xNome.valor = (
                inv.partner_id.legal_name[:60] or '')
        if inv.partner_id.is_company:
            self.nfe.infNFe.dest.CNPJ.valor = re.sub(
                '[%s]' % re.escape(string.punctuation), '',
                inv.partner_id.cnpj_cpf or '')
            self.nfe.infNFe.dest.IE.valor = re.sub(
                '[%s]' % re.escape(string.punctuation), '',
                inv.partner_id.inscr_est or '')
        else:
            self.nfe.infNFe.dest.CPF.valor = re.sub(
                '[%s]' % re.escape(string.punctuation), '',
                inv.partner_id.cnpj_cpf or '')
        ...
        self.nfe.infNFe.dest.enderDest.cMun.valor = '%s%s' % (
        ...
        self.nfe.infNFe.dest.enderDest.fone.valor = re.sub(
            '[%s]' % re.escape(string.punctuation), '',
    ...
    def _details(self, cr, uid, ids, inv, inv_line, i, context=None):
        ...
        self.det.prod.NCM.valor = re.sub(
            '[%s]' % re.escape(string.punctuation),
            '',
            inv_line.fiscal_classification_id.name or '')[:8]
        ...
        if inv_line.product_type == 'product':
            ...
            self.det.imposto.ICMS.pRedBCST.valor = str(
                "%.2f" % inv_line.icms_st_percent_reduction)
        ...
        self.det.imposto.COFINSST.vBC.valor = str(
            "%.2f" % inv_line.cofins_st_base)
        self.det.imposto.COFINSST.pCOFINS.valor = str(
            "%.2f" % inv_line.cofins_st_percent)
    ...
    def _addition(self, cr, uid, ids, inv, inv_line, inv_di, inv_di_line, i,
                  context=None):
    ...
    def _encashment_data(self, cr, uid, ids, inv, line, context=None):
        ...
        self.dup.dVenc.valor = (
            line.date_maturity or
            inv.date_due or
            inv.date_invoice
        )
    ...
    def _carrier_data(self, cr, uid, ids, inv, context=None):
        ...
        self.nfe.infNFe.transp.modFrete.valor = (
            inv.incoterm and
            inv.incoterm.freight_responsibility or '9'
        )
        if inv.carrier_id:
            ...
            self.nfe.infNFe.transp.transporta.xNome.valor = (
                inv.carrier_id.partner_id.legal_name[:60] or '')
            self.nfe.infNFe.transp.transporta.IE.valor = (
